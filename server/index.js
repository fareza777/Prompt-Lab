...
if (isDirectRun) {
  app.listen(port, () => {
    console.log(`AI Work Studio API running at http://127.0.0.1:${port}`);
  });
}
...
- Jangan masukkan judul engine internal ke output final.
${targetGuidance}
...
function getOptimizerEngineInstruction(payload) {
  ...
  return `Internal Optimizer Engine:
...
function buildCompareInstruction(payload) {
  const targetGuidance = getTargetModelGuidance(payload.targetModel, payload);
  return `Act as a neutral internal comparison judge.
...
function getDeliverableGuard(payload, attachments) {
  ...
  const lines = [
    `Jenis output terpilih dari UI: ${explicitOutput}. Prioritaskan ini sebagai sumber kebenaran utama jika narasi ambigu.`,
...
function getIntentEngineInstruction(payload, attachments) {
  ...
  return `Engine intent internal wajib dipakai sebelum menulis instruksi final:
...
Jangan masukkan judul engine intent internal ke output final.
...
- Jangan tampilkan bagian engine optimizer internal di output final provider; gunakan hanya sebagai logika internal.
...
function buildSlidesFromContent(pptx, title, content) {
  ...
  titleSlide.addText("Generated by AI Work Studio", {
    x: 0.75,
    y: 2.9,
    w: 8,
    h: 0.4,
...
function splitContentIntoSlideSections(content) {
  ...
  if (sections.length === 0) sections.push({ title: "AI Work Studio Export", body: ["Tidak ada konten untuk diekspor."] });
